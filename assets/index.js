...
.label.disabled {
    color: #a8a8a8;
}`,_e=class extends i{configContext;metagoraContext;constructor(){super(`plain-catalogue-item`,ge),this.configContext=this.useContext(m.CONFIG,!0),this.metagoraContext=this.useContext(m.METAGORA,!0)}template(){let e=this.configContext.get(`IS_METAGORA`),t=e?(this.metagoraContext.get(`agoras`)||[]).find(e=>e.name===this.props.companyName):void 0,n=`${this.props.website||(e?t?.host:this.configContext.get(`API_HOST`))}${this.props.url}`;return e&&!t?this.html`
...
`}</span>
        ...
        `}};window.customElements.define(`plain-catalogue-item`,_e);var ve=class extends i{serviceContext;companyContext;constructor(){super(`plain-nav-menu`,re),this.serviceContext=this.useContext(m.SERVICE,!0),this.companyContext=this.useContext(m.COMPANY,!0)}renderSkeletonLoader(){let e=[,,,,].fill(null).map((e,t)=>this.html`
        ...
        `}template(){let e=this.serviceContext.get(`services`);return!e||e.length===0?this.renderSkeletonLoader():this.html`
            ${e.map(t=>{let n=t.fields.catalogues.websites,r=n&&n.length>0,i=t.fields.main_url&&t.fields.main_url!==`False`?t.fields.main_url:``,a=this.html`
                ...
                `:this.html`
                    ...
                    `}).join(`
`)}
        `}};window.customElements.define(`plain-nav-menu`,ve);var ye=`.plain-metagora-nav-menu-wrapper {
    width: 100%;
    height: fit-content;
} 
...
} `;function be(e){if(!e||typeof e!=`string`)return null;try{let t=[],n=e.replace(/\bTrue\b/g,`true`).replace(/\bFalse\b/g,`false`).replace(/\bNone\b/g,`null`).replace(/"((?:[^"\\]|\\.)*)"/g,(e,n)=>(t.push(n),`<<<${t.length-1}>>>`)).replace(/'((?:[^'\\]|\\.)*)'/g,(e,t)=>`"${t.replace(/\\'/g,`'`).replace(/"/g,`\\"`)}"`).replace(/<<<(\d+)>>>/g,(e,n)=>`"${t[parseInt(n)]}"`);return JSON.parse(n)}catch(t){return console.warn(`Failed to parse Python object to JSON:`,e,t),null}}function xe(e){if(!e||typeof e!=`string`||!e.startsWith(`[`)||e===`[]`)return null;let t=be(e);if(!t)return null;if(Array.isArray(t)&&t.length>0){let e=t[0];if(typeof e==`object`&&e&&`name`in e)return t.map(e=>e.name||e.display_name||``).filter(e=>e!==``)}return null}function x(e){if(!e)return[];if(Array.isArray(e))return e.map(e=>{if(typeof e==`object`&&e){let t=e.name||e.display_name||``;if(!t)return null;let n=t.match(/^\(([^)]+)\)/);return n?n[1]:t}if(typeof e==`string`){let t=e.match(/^\(([^)]+)\)/);return t?t[1]:e}return null}).filter(e=>e!==null&&e!==``);if(typeof e==`string`){if(e.startsWith(`[`)){let t=be(e);if(t)return x(t)}let t=e.match(/^\(([^)]+)\)/);return t?[t[1]]:[e]}if(typeof e==`object`&&e){let t=e.name||e.display_name||``;if(!t)return[];let n=t.match(/^\(([^)]+)\)/);return n?[n[1]]:[t]}return[]}function S(e){return e&&e.startsWith(`as_`)?e.substring(3).replace(/_/g,` `).split(` `).map(e=>e.charAt(0).toUpperCase()+e.slice(1)).join(` `):`Other`}function Se(e){if(!e)return``;let t=document.createElement(`div`);return t.innerHTML=e,t.textContent||t.innerText||``}var Ce=class extends i{serviceContext;metagoraContext;searchQuery=``;selectedCompany=null;searchTimeout=null;constructor(){super(`plain-metagora-nav-menu`,ye),this.serviceContext=this.useContext(m.SERVICE,!0),this.metagoraContext=this.useContext(m.METAGORA,!0)}handleSearch(e){let t=e.target.value.toLowerCase();this.searchTimeout&&clearTimeout(this.searchTimeout),this.searchTimeout=window.setTimeout(()=>{this.searchQuery=t,this.updateList()},300)}handleCompanyFilter(e){this.selectedCompany===e?this.selectedCompany=null:this.selectedCompany=e,this.updateList(),this.updateFilterChips()}updateList(){let e=this.$(`#services-list`);e&&(e.innerHTML=this.getServicesListHTML())}updateFilterChips(){(this.shadowRoot?.querySelectorAll(`.filter-chip`))?.forEach(e=>{let t=e.getAttribute(`data-company`);t===this.selectedCompany||t===`all`&&this.selectedCompany===null?e.classList.add(`active`):e.classList.remove(`active`)})}renderSkeletonLoader(){let e=[,,,,].fill(null).map((e,t)=>this.html`
            <div class="skeleton-item">
                <div class="skeleton-icon"></div>
                <div class="skeleton-text" style="width: ${70+t*10%30}%"></div>
            </div>
...
}`;function Me(e,t){return e?e.startsWith(`http://`)||e.startsWith(`https://`)?e:`${t}${e}`:``}function Ne(e,t,n,r,i){if(!e||!t)return i||`#`;let a=n.filter(e=>e._sourceHost===r);for(let n of a){let i=n.fields?.catalogues?.websites||[];for(let n of i)if(n.model===e)return`${r}/offering/${n.view_id}/${t}`}for(let i of n){let n=i.fields?.catalogues?.websites||[];for(let i of n)if(i.model===e)return`${r}/offering/${i.view_id}/${t}`}return i||`#`}function Pe(e,t=1){let n=e.startsWith(`#`)?e.slice(1):e,r=parseInt(n.slice(0,2),16),i=parseInt(n.slice(2,4),16),a=parseInt(n.slice(4,6),16);return`rgba(${r}, ${i}, ${a}, ${t})`}function Fe(e,t,n){if(!e||!t||t.length===0)return e;let r=e;return[...t].sort((e,t)=>t.length-e.length).forEach(e=>{if(!e)return;let t=e.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),i=RegExp(`\\b(${t}\\w*)`,`gi`),a=n.startsWith(`#`)?Pe(n,.2):`${n}33`;r=r.replace(i,`<mark style="background-color: ${a};">$1</mark>`)}),r}var Ie=`.score-badge {
    display: flex;
    align-items: center;
    gap: 0.375rem;
    background-color: #f3f4f6;
...
.toast-item.success .toast-progress {
    background: rgba(255, 255, 255, 0.4);
} `,ot={name:`agora-home`,private:!0,version:`1.0.0`,type:`module`,scripts:{dev:`vite`,build:`tsc && vite build`,preview:`vite preview`,"create-component":`node ./scripts/create.cjs`},devDependencies:{"plain-ts":`npm:plain-ts@latest`,typescript:`~5.9.3`,vite:`npm:rolldown-vite@7.1.14`},overrides:{vite:`npm:rolldown-vite@7.1.14`},dependencies:{marked:`^17.0.1`}};const w={APP_NAME:ot.name,VERSION:ot.version,API_HOST:`http://localhost:8010`,AI_HOST:`http://localhost:2020`,TRANSLATION_HOST:null,ENABLED_AI:!0,DEBUG_MODE:!1,IS_METAGORA:!1,MAX_HEIGHT:null,VISIBLE_LOGO:!0};var st=class extends i{constructor(){super(`toast-container`,at)}template(){return this.html`
            ...
            <div class="modal-overlay">
                <div class="modal-content">
                    ...
                    <div class="modal-content--bottom">
                        ...
                        ${Object.entries(l).map(([e,n])=>{if(this.excludedFields.includes(e)||typeof n!=`string`||!n||n===`False`||n===`[]`||n===`{}`)return``;let r=e.replace(/^x_/,``).replace(/_/g,` `),i=xe(n);return i===null?this.html`
...
}`;function wt(){return{async:!1,breaks:!1,extensions:null,gfm:!0,hooks:null,pedantic:!1,renderer:null,silent:!1,tokenizer:null,walkTokens:null}}var A=wt();function Tt(e){A=e}var j={exec:()=>null};function M(e,t=``){let n=typeof e==`string`?e:e.source,r={replace:(e,t)=>{let i=typeof t==`string`?t:t.source;return i=i.replace(N.caret,`$1`),n=n.replace(e,i),r},getRegex:()=>new RegExp(n,t)};return r}var Et=(()=>{try{return!0}catch{return!1}})(),N={codeRemoveIndent:/^(?: {1,4}| {0,3}\t)/gm,outputLinkReplace:/\\([\[\]])/g,indentCodeCompensation:/^(\s+)(?:```)/,beginningSpace:/^\s+/,endingHash:/#$/,startingSpaceChar:/^ /,endingSpaceChar:/ $/,nonSpaceChar:/[^ ]/,newLineCharGlobal:/\n/g,tabCharGlobal:/\t/g,multipleSpaceGlobal:/\s+/g,blankLine:/^[ \t]*$/,doubleBlankLine:/\n[ \t]*\n[ \t]*$/,blockquoteStart:/^ {0,3}>/,blockquoteSetextReplace:/\n {0,3}((?:=+|-+) *)(?=\n|$)/g,blockquoteSetextReplace2:/^ {0,3}>[ \t]?/gm,listReplaceTabs:/^\t+/,listReplaceNesting:/^ {1,4}(?=( {4})*[^ ])/g,listIsTask:/^\[[ xX]\] +\S/,listReplaceTask:/^\[[ xX]\] +/,listTaskCheckbox:/\[[ xX]\]/,anyLine:/\n.*\n/,hrefBrackets:/^<(.*)>$/,tableDelimiter:/[:|]/,tableAlignChars:/^\||\| *$/g,tableRowBlankLine:/\n[ \t]*$/,tableAlignRight:/^ *-+: *$/,tableAlignCenter:/^ *:-+: *$/,tableAlignLeft:/^ *:-+ *$/,startATag:/^<a /i,endATag:/^<\/a>/i,startPreScriptTag:/^<(pre|code|kbd|script)(\s|>)/i,endPreScriptTag:/^<\/(pre|code|kbd|script)(\s|>)/i,startAngleBracket:/^</,endAngleBracket:/>$/,pedanticHrefTitle:/^([^'"]*[^\s])\s+(['"])(.*)\2/,unicodeAlphaNumeric:/[\p{L}\p{N}]/u,escapeTest:/[&<>"']/,escapeReplace:/[&<>"']/g,escapeTestNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,escapeReplaceNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/g,unescapeTest:/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/gi,caret:/(^|[^\[])\^/g,percentDecode:/%25/g,findPipe:/\|/g,splitPipe:/ \|/,slashPipe:/\\\|/g,carriageReturn:/\r\n|\r/g,spaceLine:/^ +$/gm,notSpaceStart:/^\S*/,endingNewline:/\n$/,listItemRegex:e=>RegExp(`^( {0,3}${e})((?:[	 ][^\\n]*)?(?:\\n|$))`),nextBulletRegex:e=>RegExp(`^ {0,${Math.min(3,e-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),hrRegex:e=>RegExp(`^ {0,${Math.min(3,e-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),fencesBeginRegex:e=>RegExp(`^ {0,${Math.min(3,e-1)}}(?:\`\`\`|~~~)`),headingBeginRegex:e=>RegExp(`^ {0,${Math.min(3,e-1)}}#`),htmlBeginRegex:e=>RegExp(`^ {0,${Math.min(3,e-1)}}<(?:[a-z].*>|!--)`,`i`)},Dt=/^(?:[ \t]*(?:\n|$))+/,Ot=/^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/,kt=/^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/,P=/^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,At=/^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/,F=/(?:[*+-]|\d{1,9}[.)])/,jt=/^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/,Mt=M(jt).replace(/bull/g,F).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/\|table/g,``).getRegex(),Nt=M(jt).replace(/bull/g,F).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/table/g,/ {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),I=/^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/,Pt=/^[^\n]+/,L=/(?!\s*\])(?:\\[\s\S]|[^\[\]\\])+/,Ft=M(/^ {0,3}\[(label)\]: *(?:\n[ \t]*)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n[ \t]*)?| *\n[ \t]*)(title))? *(?:\n+|$)/).replace(`label`,L).replace(`title`,/(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(),It=M(/^( {0,3}bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g,F).getRegex(),R=`address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul`,z=/<!--(?:-?>|[\s\S]*?(?:-->|$))/,Lt=M(`^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))`,`i`).replace(`comment`,z).replace(`tag`,R).replace(`attribute`,/ +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),Rt=M(I).replace(`hr`,P).replace(`heading`,` {0,3}#{1,6}(?:\\s|$)`).replace(`|lheading`,``).replace(`|table`,``).replace(`blockquote`,` {0,3}>`).replace(`fences`," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace(`list`,` {0,3}(?:[*+-]|1[.)]) `).replace(`html`,`</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)`).replace(`tag`,R).getRegex(),B={blockquote:M(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace(`paragraph`,Rt).getRegex(),code:Ot,def:Ft,fences:kt,heading:At,hr:P,html:Lt,lheading:Mt,list:It,newline:Dt,paragraph:Rt,table:j,text:Pt},zt=M(`^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)`).replace(`hr`,P).replace(`heading`,` {0,3}#{1,6}(?:\\s|$)`).replace(`blockquote`,` {0,3}>`).replace(`code`,`(?: {4}| {0,3}	)[^\\n]`).replace(`fences`," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace(`list`,` {0,3}(?:[*+-]|1[.)]) `).replace(`html`,`</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)`).replace(`tag`,R).getRegex(),Bt={...B,lheading:Nt,table:zt,paragraph:M(I).replace(`hr`,P).replace(`heading`,` {0,3}#{1,6}(?:\\s|$)`).replace(`|lheading`,``).replace(`table`,zt).replace(`blockquote`,` {0,3}>`).replace(`fences`," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace(`list`,` {0,3}(?:[*+-]|1[.)]) `).replace(`html`,`</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)`).replace(`tag`,R).getRegex()},Vt={...B,html:M(`^ *(?:comment *(?:\\n|\\s*$)|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)|<tag(?:"[^"]*"|'[^']*'|\\s[^'"/>\\s]*)*?/?> *(?:\\n{2,}|\\s*$))`).replace(`comment`,z).replace(/tag/g,`(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b`).getRegex(),def:/^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +(["(][^\n]+[")]))? *(?:\n+|$)/,heading:/^(#{1,6})(.*)(?:\n+|$)/,fences:j,lheading:/^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,paragraph:M(I).replace(`hr`,P).replace(`heading`,` *#{1,6} *[^
]`).replace(`lheading`,Mt).replace(`|table`,``).replace(`blockquote`,` {0,3}>`).replace(`|fences`,``).replace(`|list`,``).replace(`|html`,``).replace(`|tag`,``).getRegex()},Ht=/^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/,Ut=/^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/,Wt=/^( {2,}|\\)\n(?!\s*$)/,Gt=/^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/,V=/[\p{P}\p{S}]/u,Kt=/[\s\p{P}\p{S}]/u,qt=/[^\s\p{P}\p{S}]/u,Jt=M(/^((?![*_])punctSpace)/,`u`).replace(/punctSpace/g,Kt).getRegex(),Yt=/(?!~)[\p{P}\p{S}]/u,Xt=/(?!~)[\s\p{P}\p{S}]/u,Zt=/(?:[^\s\p{P}\p{S}]|~)/u,Qt=M(/link|precode-code|html/,`g`).replace(`link`,/\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace(`precode-`,Et?"(?<!`)()":"(^^|[^`])").replace(`code`,/(?<b>`+)[^`]+\k<b>(?!`)/).replace(`html`,/<(?! )[^<>]*?>/).getRegex(),$t=/^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,en=M($t,`u`).replace(/punct/g,V).getRegex(),tn=M($t,`u`).replace(/punct/g,Yt).getRegex(),nn=`^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)`,rn=M(nn,`gu`).replace(/notPunctSpace/g,qt).replace(/punctSpace/g,Kt).replace(/punct/g,V).getRegex(),an=M(nn,`gu`).replace(/notPunctSpace/g,Zt).replace(/punctSpace/g,Xt).replace(/punct/g,Yt).getRegex(),on=M(`^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)`,`gu`).replace(/notPunctSpace/g,qt).replace(/punctSpace/g,Kt).replace(/punct/g,V).getRegex(),sn=M(/\\(punct)/,`gu`).replace(/punct/g,V).getRegex(),cn=M(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace(`scheme`,/[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace(`email`,/[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),ln=M(z).replace(`(?:-->|$)`,`-->`).getRegex(),un=M(`^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>`).replace(`comment`,ln).replace(`attribute`,/\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),H=/(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/,dn=M(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace(`label`,H).replace(`href`,/<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace(`title`,/"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),fn=M(/^!?\[(label)\]\[(ref)\]/).replace(`label`,H).replace(`ref`,L).getRegex(),pn=M(/^!?\[(ref)\](?:\[\])?/).replace(`ref`,L).getRegex(),mn=M(`reflink|nolink(?!\\()`,`g`).replace(`reflink`,fn).replace(`nolink`,pn).getRegex(),hn=/[hH][tT][tT][pP][sS]?|[fF][tT][pP]/,gn={_backpedal:j,anyPunctuation:sn,autolink:cn,blockSkip:Qt,br:Wt,code:Ut,del:j,emStrongLDelim:en,emStrongRDelimAst:rn,emStrongRDelimUnd:on,escape:Ht,link:dn,nolink:pn,punctuation:Jt,reflink:fn,reflinkSearch:mn,tag:un,text:Gt,url:j},_n={...gn,link:M(/^!?\[(label)\]\((.*?)\)/).replace(`label`,H).getRegex(),reflink:M(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace(`label`,H).getRegex()},vn={...gn,emStrongRDelimAst:an,emStrongLDelim:tn,url:M(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace(`protocol`,hn).replace(`email`,/[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),_backpedal:/(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,del:/^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/,text:M(/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|protocol:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/).replace(`protocol`,hn).getRegex()},yn={...vn,br:M(Wt).replace(`{2,}`,`*`).getRegex(),text:M(vn.text).replace(`\\b_`,`\\b_| {2,}\\n`).replace(/\{2,\}/g,`*`).getRegex()},U={normal:B,gfm:Bt,pedantic:Vt},W={normal:gn,gfm:vn,breaks:yn,pedantic:_n},bn={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},xn=e=>bn[e];function G(e,t){if(t){if(N.escapeTest.test(e))return e.replace(N.escapeReplace,xn)}else if(N.escapeTestNoEncode.test(e))return e.replace(N.escapeReplaceNoEncode,xn);return e}function Sn(e){try{e=encodeURI(e).replace(N.percentDecode,`%`)}catch{return null}return e}function Cn(e,t){let n=e.replace(N.findPipe,(e,t,n)=>{let r=!1,i=t;for(;--i>=0&&n[i]===`\\`;)r=!r;return r?`|`:` |`}).split(N.splitPipe),r=0;if(n[0].trim()||n.shift(),n.length>0&&!n.at(-1)?.trim()&&n.pop(),t)if(n.length>t)n.splice(t);else for(;n.length<t;)n.push(``);for(;r<n.length;r++)n[r]=n[r].trim().replace(N.slashPipe,`|`);return n}function K(e,t,n){let r=e.length;if(r===0)return``;let i=0;for(;i<r;){let a=e.charAt(r-i-1);if(a===t&&!n)i++;else if(a!==t&&n)i++;else break}return e.slice(0,r-i)}function wn(e,t){if(e.indexOf(t[1])===-1)return-1;let n=0;for(let r=0;r<e.length;r++)if(e[r]===`\\`)r++;else if(e[r]===t[0])n++;else if(e[r]===t[1]&&(n--,n<0))return r;return n>0?-2:-1}function Tn(e,t,n,r,i){let a=t.href,o=t.title||null,s=e[1].replace(i.other.outputLinkReplace,`$1`);r.state.inLink=!0;let c={type:e[0].charAt(0)===`!`?`image`:`link`,raw:n,href:a,title:o,text:s,tokens:r.inlineTokens(s)};return r.state.inLink=!1,c}function En(e,t,n){let r=e.match(n.other.indentCodeCompensation);if(r===null)return t;let i=r[1];return t.split(`
`).map(e=>{let t=e.match(n.other.beginningSpace);if(t===null)return e;let[r]=t;return r.length>=i.length?e.slice(i.length):e}).join(`
`)}var q=class{options;rules;lexer;constructor(e){this.options=e||A}space(e){let t=this.rules.block.newline.exec(e);if(t&&t[0].length>0)return{type:`space`,raw:t[0]}}code(e){let t=this.rules.block.code.exec(e);if(t){let e=t[0].replace(this.rules.other.codeRemoveIndent,``);return{type:`code`,raw:t[0],codeBlockStyle:`indented`,text:this.options.pedantic?e:K(e,`
